...
// **********************************************************************************
#include <RFM69_Sh.h>
#include <RFM69registers_Sh.h>
#include <SPI.h>

volatile uint8_t RFM69_Sh::DATA[RF69_MAX_DATA_LEN];
volatile uint8_t RFM69_Sh::_mode;        // current transceiver state
volatile uint8_t RFM69_Sh::DATALEN;
volatile uint8_t RFM69_Sh::SENDERID;
volatile uint8_t RFM69_Sh::TARGETID;     // should match _address
volatile uint8_t RFM69_Sh::PAYLOADLEN;
volatile uint8_t RFM69_Sh::ACK_REQUESTED;
volatile uint8_t RFM69_Sh::ACK_RECEIVED; // should be polled immediately after sending a packet with ACK request
volatile int16_t RFM69_Sh::RSSI;          // most accurate RSSI during reception (closest to the reception)
RFM69_Sh* RFM69_Sh::selfPointer;

bool RFM69_Sh::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  const uint8_t CONFIG[][2] =
  {
    /* 0x01 */ { REG_OPMODE, RF_OPMODE_SEQUENCER_ON | RF_OPMODE_LISTEN_OFF | RF_OPMODE_STANDBY },
    /* 0x02 */ { REG_DATAMODUL, RF_DATAMODUL_DATAMODE_PACKET | RF_DATAMODUL_MODULATIONTYPE_FSK | RF_DATAMODUL_MODULATIONSHAPING_00 }, // no shaping
    /* 0x03 */ { REG_BITRATEMSB, RF_BITRATEMSB_55555}, // default: 55.555 KBPS
  ...
  if (millis()-start >= timeout)
    return false;
  attachInterrupt(_interruptNum, RFM69_Sh::isr0, RISING);
...
uint32_t RFM69_Sh::getFrequency()
{
  return RF69_FSTEP * (((uint32_t) readReg(REG_FRFMSB) << 16) + ((uint16_t) readReg(REG_FRFMID) << 8) + readReg(REG_FRFLSB));
}

// set the frequency (in Hz)
void RFM69_Sh::setFrequency(uint32_t freqHz)
{
...
void RFM69_Sh::setMode(uint8_t newMode)
{
  if (newMode == _mode)
    return;
...
void RFM69_Sh::setAddress(uint8_t addr)
{
  _address = addr;
...
void RFM69_Sh::setNetwork(uint8_t networkID)
{
...
void RFM69_Sh::setPowerLevel(uint8_t powerLevel)
{
...
void RFM69_Sh::send(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK)
{
...
bool RFM69_Sh::sendWithRetry(uint8_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries, uint8_t retryWaitTime) {
  uint32_t sentTime;
...
bool RFM69_Sh::ACKReceived(uint8_t fromNodeID) {
  if (receiveDone())
...
bool RFM69_Sh::ACKRequested() {
  return ACK_REQUESTED && (TARGETID != RF69_BROADCAST_ADDR);
}

// should be called immediately after reception in case sender wants ACK
void RFM69_Sh::sendACK(const void* buffer, uint8_t bufferSize) {
...
// internal function
void RFM69_Sh::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
...
void RFM69_Sh::interruptHandler() {
  //pinMode(4, OUTPUT);
  //digitalWrite(4, 1);
...
// internal function
void RFM69_Sh::isr0() { selfPointer->interruptHandler(); }

// internal function
void RFM69_Sh::receiveBegin() {
  DATALEN = 0;
  SENDERID = 0;
  TARGETID = 0;
  PAYLOADLEN = 0;
...
void RFM69_Sh::encrypt(const char* key) {
  setMode(RF69_MODE_STANDBY);
  if (key != 0)
  {
    select();
...
int16_t RFM69_Sh::readRSSI(bool forceTrigger) {
  int16_t rssi = 0;
  if (forceTrigger)
  {
...
uint8_t RFM69_Sh::readReg(uint8_t addr)
{
  select();
...
void RFM69_Sh::writeReg(uint8_t addr, uint8_t value)
{
  select();
...
void RFM69_Sh::promiscuous(bool onOff) {
  _promiscuousMode = onOff;
  //writeReg(REG_PACKETCONFIG1, (readReg(REG_PACKETCONFIG1) & 0xF9) | (onOff ? RF_PACKET1_ADRSFILTERING_OFF : RF_PACKET1_ADRSFILTERING_NODEBROADCAST));
}

// for RFM69HW only: you must call setHighPower(true) after initialize() or else transmission won't work
void RFM69_Sh::setHighPower(bool onOff) {
  _isRFM69HW = onOff;
...
// internal function
void RFM69_Sh::setHighPowerRegs(bool onOff) {
  writeReg(REG_TESTPA1, onOff ? 0x5D : 0x55);
  writeReg(REG_TESTPA2, onOff ? 0x7C : 0x70);
}

// set the slave select (CS) pin 
void RFM69_Sh::setCS(uint8_t newSPISlaveSelect) {
...
uint8_t RFM69_Sh::readTemperature(uint8_t calFactor) // returns centigrade
{
  setMode(RF69_MODE_STANDBY);
